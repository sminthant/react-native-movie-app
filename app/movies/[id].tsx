...
export default function MovieDetailScreen() {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create(
        createMovieDetailStyles(colors) as Parameters<
  ...
  useEffect(() => {
    ...
    getSavedByMovieId(movieId, userId)
      .then((doc) => {
        setSavedDoc(doc);
        setSaved(!!doc);
      })
  ...
  const toggleSaved = useCallback(async () => {
    ...
    try {
      if (saved && savedDoc) {
        ...
        const doc = await createSavedMovie(movieId, userId);
        setSaved(true);
        setSavedDoc(doc);
      }
    } catch (err) {
  ...
  }, [id, saved, savedDoc, saving, userId]);
